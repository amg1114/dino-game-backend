...
export class UpdateResultResponseDto {
    @ApiProperty({ description: 'Número de filas afectadas por la actualización', example: 1 })
    affected?: number;

    @ApiProperty({ description: 'Resultado crudo de la consulta SQL ejecutada', example: [] })
    raw: any;

    @ApiProperty({
        description: 'Valores generados por la base de datos durante la actualización',
        example: [],
    })
...
export class DeleteResultResponseDto {
    @ApiProperty({ description: 'Número de filas afectadas por la eliminación', example: 1 })
...
export class GetRoleResponseDto {
    @ApiProperty({ enum: Role, isArray: true, example: [Role.ADMINISTRATOR, Role.DEVELOPER] })
    roles: Role[];
}
